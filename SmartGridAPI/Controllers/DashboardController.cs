        ...
        public async Task<IActionResult> GenerateReport([FromQuery] DateTime? startDate, [FromQuery] DateTime? endDate)
        {
            try
            {
                ...
                var metrics = new Dictionary<string, object>
                {
                    ...
                    ["Avg Voltage (V)"] = Math.Round(readings.Select(r => (double)r.Voltage).DefaultIfEmpty(0).Average(), 2),
                    ["Avg Power Factor"] = Math.Round(readings.Select(r => (double)r.PowerFactor).DefaultIfEmpty(0).Average(), 3),
